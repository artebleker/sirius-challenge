...
const TableContainer = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i <= 42; i++) {
      endPointsCharacters.push(`https://rickandmortyapi.com/api/character/?page=${i}`);
    }
    ...
    axios
      ...
      .then((data) => {
        ...
        let sortArray = Array.from(new Set(finalResult.map( a => a.id))).map(id => { return finalResult.find(a => a.id === id)})
              ...
              <thead>
                <tr className="title tr-th">
                  ...
                  <th className="responsive-column-specie">
                    <button
                      className={`btn-modal-close text ${sortSpecies && 'btn-sort'}`}
                      onClick={(e) =>{ e.preventDefault(); setSortSpecies(!sortSpecies)}}
                    >
                      Specie
                    </button>
                  </th>
                    ...
                    return (
                      index < currentPage * 20 && (
                        ...
                        >
                          ...
                          <td className="responsive-column">{character.status}</td>
                          <td className="responsive-column-specie">{character.species}</td>
                          <td className="responsive-column">{character.gender}</td>
